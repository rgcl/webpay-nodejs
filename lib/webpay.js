...
class WebPay {
    ...
    constructor(props) {
        ...
        this._wsSecurity = new WebPayUniqueAndSpecialNonStandardWSSecurityCert(this.privateKey,
            this.publicKey, '', 'utf8', true);
    }

    _getTransactionNormalClient() {
        if(!this._transactionNormalClient) {
            return new Promise((resolve, reject) => {
                ...
                soap.createClient(this.env.normal, options, (err, client) => {
                    if(err) {
                        return reject(err);
                    }
                    this._wsSecurity.promise().then(() => {
                        client.setSecurity(this._wsSecurity);
                        this._transactionNormalClient = client;
                        resolve(this._transactionNormalClient);
                    });
                });
            });
        }
        return Promise.resolve(this._transactionNormalClient);
    }

    _getTransactionNullifyClient() {
        if(!this._transactionNullifyClient) {
            return new Promise((resolve, reject) => {
                let options = {
                    ignoredNamespaces: {
                        namespaces: [],
                ...
                soap.createClient(this.env.nullify, options, (err, client) => {
                    if(err) {
                        return reject(err);
                    }
                    this._wsSecurity.promise().then(() => {
                        client.setSecurity(this._wsSecurity);
                        this._transactionNullifyClient = client;
                        resolve(this._transactionNullifyClient);
                    });
                });
            });
        }
        return Promise.resolve(this._transactionNullifyClient);
    }
    ...
    initTransaction(props) {
        ...
        return new Promise((resolve, reject) => {
            this._getTransactionNormalClient().then((client) => {
    ...
    nullify(props) {
        ...
        return new Promise((resolve, reject) => {
            this._getTransactionNullifyClient().then((client) => {
